...
public:
    ...
    int setHeuristicValue(searchNode *n) {
        ...
        if constexpr (std::is_same_v<ClassicalHeuristic, hsPrefixMakespan>) {
            vector<int> prefix;
            solutionStep *s = n->solution;
            while(s) {
                if (s->task < htn->numActions) { // primitive
                    prefix.push_back(s->task);
                }
                s = s->prev;
            }
            std::reverse(prefix.begin(), prefix.end());
            this->sasH->setPrefix(prefix);
            this->sasH->setNodeMakespan(n->planMakespan);
        }

        if constexpr (std::is_same_v<ClassicalHeuristic, hsPrefixMakespanFast>) {
            this->sasH->setFactTimesFromNode(n->factEarliestTrue, htn->numStateBits);
            this->sasH->setNodeMakespan(n->planMakespan);
        }
