...
struct solutionStep {
	...
	int pointersToMe;
	int observationId = -1;
	std::vector<int> observationPredecessors;
#ifdef TRACESOLUTION
...
// Export machine-readable hierarchy-induced ordering constraints (and action map)
// Writes files in the current working directory:
// - panda_actions.map: "<id> <actionName>"
// - panda_order.lock: "<id_u> <id_v>" edges u->v that must be respected
// - panda_order.lock.names: "<name_u> <name_v>" name pairs for convenience
