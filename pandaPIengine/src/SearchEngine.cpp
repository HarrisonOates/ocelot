...
void printHeuristicHelp(){
	...
	cout << "    - prefixMakespan (admissible makespan heuristic; replays prefix to seed the planning graph)" << endl;
	cout << "    - prefixMakespanFast (admissible makespan heuristic; uses incremental fact times, faster than prefixMakespan)" << endl;
	...
	if (algo == INTERACTIVE){
		...
		for (int i = 0; i < hLength; i++){
			...
			if (hName == "zero"){
				...
				if (subName == "prefixMakespan" || subName == "prefixMakespanFast") {
					correctTaskCount = false;
				}
	            ...
	            } else if (subName == "prefixMakespan"){
		    		heuristics[i] = new hhRC2<hsPrefixMakespan>(htn, i, estimate, correctTaskCount);
				} else if (subName == "prefixMakespanFast"){
		    		heuristics[i] = new hhRC2<hsPrefixMakespanFast>(htn, i, estimate, correctTaskCount);
		...
		int aStarWeight = args_info.astarweight_arg;
		    ...
		    if (string(args_info.gValue_arg) == "makespan") aStarType = gValMakespan;
		...
		switch (aStarType){
			...
			case gValMakespan: cout << "plan makespan"; break;
		}
		cout << endl;
		...
		// Run progression search
	bool exportOrderings = args_info.orderOutput_given;
	search.search(htn, tnI, timeL, suboptimalSearch, printPlan, heuristics, hLength, visi, fringe, exportOrderings);

		// If requested, export implied orderings for the found solution from progression
		...
		bool outputOrder = args_info.orderOutput_given;
