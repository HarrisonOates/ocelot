...
namespace progression {
    ...
    search(Model *htn, searchNode *tnI, int timeLimit, bool suboptimalSearch, bool printSolution, Heuristic **hF,
           int hLength, VisitedList &visitedList, Fringe &fringe, bool exportOrderings = false) {
            timeval tp;
            ...
            if (tnSol != nullptr) {
                ...
                cout << "- Plan makespan: " << tnSol->planMakespan << endl;
                if (printSolution) cout << sol << endl;
                if (exportOrderings) {
                    exportImpliedOrderings(htn, tnSol);
                }
#ifdef TRACKLMSFULL
