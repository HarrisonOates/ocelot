		...
		/*	for (int i = 0; i < n->numPrimitive; i++)
			...
			*/
			for (int i = 0; i < numAdds[progressed->task]; i++) {
				...
				result->state[fact] = true;
				// Set when this fact becomes available for causal ordering
				// Effects are available at the same time the action finishes
				if (enableMakespanTrace && n->factEarliestTrue && n->factEarliestTrue[fact] != parallelInf) {
					std::cerr << "[makespan-add] " << taskNames[progressed->task]
					          << " adds " << factStrs[fact] 
					          << " (was available at " << n->factEarliestTrue[fact]
					          << ", now at " << finish << ")" << std::endl;
				}
				result->factEarliestTrue[fact] = finish;
			}
			result->planMakespan = std::max(n->planMakespan, finish);
...
*/
	...
	searchNode *Model::decompose(searchNode *n, int taskNo, int method) {
		...
		if (n->taskEarliestStart != nullptr) {
			...
			result->taskEarliestStart = new std::map<int, int>();
		}
		if (n->pendingObservationPredecessors != nullptr) {
			result->pendingObservationPredecessors = new std::unordered_map<int, std::vector<int>>(*n->pendingObservationPredecessors);
		} else {
			result->pendingObservationPredecessors = nullptr;
		}
		result->observationCount = n->observationCount;

		const int NO_INHERITED_CONSTRAINT = -1;
		int inheritedEarliestStart = NO_INHERITED_CONSTRAINT;
		if (result->taskEarliestStart != nullptr) {
			auto existing = result->taskEarliestStart->find(decomposed->id);
			if (existing != result->taskEarliestStart->end()) {
				inheritedEarliestStart = existing->second;
				result->taskEarliestStart->erase(existing);
			}
		}
		...
		for (int i = 0; i < numFirstTasks[method]; i++) {
			if (isPrimitive[mInstance.first[i]->task]) {
				...
				if (inheritedEarliestStart != NO_INHERITED_CONSTRAINT) {
					if (result->taskEarliestStart == nullptr) {
						result->taskEarliestStart = new std::map<int, int>();
					}
					auto it = result->taskEarliestStart->find(mInstance.first[i]->id);
					if (it == result->taskEarliestStart->end() || it->second < inheritedEarliestStart) {
						(*result->taskEarliestStart)[mInstance.first[i]->id] = inheritedEarliestStart;
					}
				}
				primI++;
			} else {
	...
	searchNode *Model::apply(searchNode *n, int taskNo) {
		...
		if (n->pendingObservationPredecessors != nullptr) {
			auto itPred = n->pendingObservationPredecessors->find(progressed->id);
			if (itPred != n->pendingObservationPredecessors->end()) {
				inheritedObservationPredecessors = itPred->second;
			}
		}
		...
		if (n->pendingObservationPredecessors != nullptr) {
			...
			result->pendingObservationPredecessors = new std::unordered_map<int, std::vector<int>>();
		}
		result->pendingObservationPredecessors->erase(progressed->id);
		result->observationCount = n->observationCount + 1;

		assert(isApplicable(n, progressed->task));

	// Debug: declare these early for tracing
	static bool enableMakespanTrace = getenv("TRACE_MAKESPAN") != nullptr;
	static bool enableDetailedTrace = getenv("TRACE_MAKESPAN_DETAIL") != nullptr;

	// Compute earliest start time based on:
	// 1. Precondition availability (state-based dependencies)
	// 2. HTN ordering constraints (predecessor finish times)
	// 3. Resource availability (mutex constraints)
	int earliestStart = 0;
	
	// Check state-based dependencies (preconditions)
	if (n->factEarliestTrue != nullptr) {
		for (int i = 0; i < numPrecs[progressed->task]; ++i) {
			int prec = precLists[progressed->task][i];
			int precTime = n->factEarliestTrue[prec];
			if (precTime > earliestStart) {
				earliestStart = precTime;
			}
		}
	} else if (numPrecs[progressed->task] > 0) {
		earliestStart = n->planMakespan;
	}
	
	// Check HTN ordering constraints (must wait for predecessors)
	if (n->taskEarliestStart != nullptr && n->taskEarliestStart->count(progressed->id)) {
		int orderingConstraint = (*n->taskEarliestStart)[progressed->id];
		if (orderingConstraint > earliestStart) {
			if (enableMakespanTrace) {
				std::cerr << "[makespan-ordering] " << taskNames[progressed->task]
				          << " delayed by HTN ordering from " << earliestStart 
				          << " to " << orderingConstraint << std::endl;
			}
			earliestStart = orderingConstraint;
		}
	}
	...
	// Debug: trace makespan updates (static declarations moved earlier)
	if (enableDetailedTrace && numPrecs[progressed->task] > 0) {
		std::cerr << "[makespan-detail] " << taskNames[progressed->task] << " preconditions:" << std::endl;
		for (int i = 0; i < numPrecs[progressed->task]; ++i) {
			int prec = precLists[progressed->task][i];
			std::cerr << "  " << factStrs[prec] << " @ time " << n->factEarliestTrue[prec];
			if (factStrs[prec].rfind("bur-", 0) == 0) {
				std::cerr << " (FAKE)";
			}
			std::cerr << std::endl;
		}
		std::cerr << "  => earliestStart=" << earliestStart << std::endl;
	}
	if (enableMakespanTrace) {
		std::cerr << "[makespan] apply " << taskNames[progressed->task] 
		          << " start=" << earliestStart << " finish=" << finish 
	...
	// Copy HTN ordering constraints from parent
	if (n->taskEarliestStart != nullptr) {
		result->taskEarliestStart = new std::map<int, int>(*n->taskEarliestStart);
	} else {
		result->taskEarliestStart = new std::map<int, int>();
	}		// Update earliest start times for all successors of the progressed task
		// They cannot start until this task finishes
		for (int i = 0; i < progressed->numSuccessors; i++) {
			int successorId = progressed->successorList[i]->id;
			// Update if we don't have a constraint, or if this constraint is stronger
			if (result->taskEarliestStart->count(successorId) == 0 || 
			    (*result->taskEarliestStart)[successorId] < finish) {
				(*result->taskEarliestStart)[successorId] = finish;
				if (enableMakespanTrace) {
					std::cerr << "[makespan-ordering] " << taskNames[progressed->task] 
					          << " (finishes at " << finish << ") enforces successor planStep#"
					          << successorId << " cannot start before " << finish << std::endl;
				}
			}
		}
		
		// Remove the constraint for the task we just executed (it's no longer in the unconstrained list)
		result->taskEarliestStart->erase(progressed->id);
	...
	void Model::calcSCCs() {
		...
		void Model::computeFactMetadata() {
			factIsStatic.assign(numStateBits, 1);
			factIsOrderingIndicator.assign(numStateBits, 0);

			for (int f = 0; f < numStateBits; ++f) {
				const string &name = factStrs[f];
				if (!name.empty()) {
					if (name.rfind("tdr-", 0) == 0 || name.rfind("bur-", 0) == 0) {
						factIsOrderingIndicator[f] = 1;
					}
				}
			}

			auto markDynamic = [&](int fact) {
				if (fact >= 0 && fact < numStateBits) {
					factIsStatic[fact] = 0;
				}
			};
			...
			for (int a = 0; a < numActions; ++a) {
				for (int i = 0; i < numAdds[a]; ++i) {
					markDynamic(addLists[a][i]);
				}
				for (int i = 0; i < numDels[a]; ++i) {
					markDynamic(delLists[a][i]);
				}
				if (numConditionalAdds[a] > 0 && conditionalAddLists[a]) {
					for (int i = 0; i < numConditionalAdds[a]; ++i) {
						markDynamic(conditionalAddLists[a][i]);
					}
				}
				if (numConditionalDels[a] > 0 && conditionalDelLists[a]) {
					for (int i = 0; i < numConditionalDels[a]; ++i) {
						markDynamic(conditionalDelLists[a][i]);
					}
				}
			}
		}
